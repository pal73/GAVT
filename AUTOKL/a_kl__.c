...
if (ind==iMn)
	...
	if(wrk_state[0]==wsON)sub_bgnd("ВКЛ.",'!',0);
	else sub_bgnd("ВЫКЛ.",'!',0);  
	
	if(wrk_state[1]==wsON)sub_bgnd("ВКЛ.",'@',0);
	else sub_bgnd("ВЫКЛ.",'@',0);
	
	if(wrk_state[2]==wsON)sub_bgnd("ВКЛ.",'#',0);
	else sub_bgnd("ВЫКЛ.",'#',0);
	
	if(wrk_state[3]==wsON)sub_bgnd("ВКЛ.",'$',0);
	else sub_bgnd("ВЫКЛ.",'$',0);			
	}	
	...
	ptrs[2]=" Tуст.       >gC";
	ptrs[3]=" Время:    (ч0)м"; 
	ptrs[4]=" Режим времени <";
	if(wrk_state[sub_ind1]==wsON)ptrs[5]=" Стоп           "; 
	else ptrs[5]=" Старт          ";	
	ptrs[6]=" Выход          ";
	ptrs[7]=" Калибровка     "; 
	if(index_set>sub_ind)index_set=sub_ind;
 ...
 /*	int2lcdxy(adc_bank_[const_of_adc[sub_ind1]],0xf0,0);
	...
	ptrs[0]=" T!      @gC    ";
	ptrs[1]=" Выход          ";
	bgnd_par("   КАЛИБРОВКА   ",ptrs[sub_ind]);
	int2lcd(sub_ind1+1,'!',0); 
	if(nd[sub_ind1])sub_bgnd("Н.Д. ",'@',-2);
	else 
		{
