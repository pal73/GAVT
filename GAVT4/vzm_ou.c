...
if(prog==p1)
	...
	if(step==sOFF)
		...
		{
		temp|=(1<<PP1)|(1<<NET)|(1<<NET_REZ);
		}

	else if((step==s4)||(step==s5))
		{
         	temp|=(1<<PP1)|(1<<PP4)|(1<<NET)|(1<<NET_REZ);
		}

	else if((step==s6)||(step==s7))
		{
		temp|=(1<<PP1)|(1<<PP5)|(1<<NET)|(1<<NET_REZ);
		}
...
if(bON_START)
	...
	if((step==sOFF)&&(!bERR))
		{
		step=s1;
		if(prog==p1) cnt_del=70;
		else if(prog==p2) cnt_del=70;
		else if(prog==p3) cnt_del=70;
