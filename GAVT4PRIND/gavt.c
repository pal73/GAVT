//Программа автомата с 5 реле, работает с платой индикации
// показывающей 2 задержки...
...
#define EE_PROGRAM 8
#define EE_PROGRAM1 10
#define EE_PROGRAM2 12
...
const unsigned DELAY_ADR_CONST[4][2]={DELAY1,DELAY2,DELAY3,DELAY4,DELAY5,DELAY6,DELAY7,DELAY8};
...
char enum{sOFF,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s100}step=sOFF;
char enum{p1=1,p2=2,p3=3,p4=4,pOFF=77}prog=p1,temp_prog;
...
////	SPBRG = 25;	/* set the baud rate 9600@4000000*/
	SPBRG = 51;	/* set the baud rate 9600@8000000*/
...
lbl_0001:
		...
		{
		temp|=(1<<PP1)|(1<<PP2)|(1<<PP3)|(1<<PP4)|(1<<PP6);
          cnt_del--;
          if(cnt_del==0)
			{
          	step=s4;
		...
		{
		temp|=(1<<PP1)|(1<<PP2)|(1<<PP3)|(1<<PP4)|(1<<PP5)|(1<<PP6);
		...
		{
		temp|=(1<<PP1)|(1<<PP2)|(1<<PP3)|(1<<PP4)|(1<<PP6)|(1<<PP7);
		...
		temp|=(1<<PP1)|(1<<PP2)|(1<<PP3)|(1<<PP4)|(1<<PP6);
          ...
          if(cnt_del==0)
			{
          	step=s8;
          	cnt_del=eeprom_read(DELAY5)*10U;
			}
          }
	else if(step==s8)
		{
...
lbl_0002:
		...
		temp|=(1<<PP1)|(1<<PP2)|(1<<PP3)|(1<<PP4)|(1<<PP6);
          ...
          if(cnt_del==0)
			{
          	step=s6;
          	cnt_del=eeprom_read(DELAY7)*10U;
			}
          }
	else if(step==s6)
		{
...
temp=eeprom_read(EE_PROGRAM);
temp1=eeprom_read(EE_PROGRAM1);
temp2=eeprom_read(EE_PROGRAM2);
...
if(temp!=eeprom_read(EE_PROGRAM))eeprom_write(EE_PROGRAM,temp);
if(temp!=eeprom_read(EE_PROGRAM1))eeprom_write(EE_PROGRAM1,temp);
if(temp!=eeprom_read(EE_PROGRAM2))eeprom_write(EE_PROGRAM2,temp);
...
if((!(in_word&0x0004))&&(main_cnt<15))
	{
	char temp;
	if(cnt_but_STOP1<500)
		{
		cnt_but_STOP1++;
		if(cnt_but_STOP1>=500)
			{
			temp=prog;//eeprom_read(EE_PROGRAM);
			temp++;
			if(!((temp<=MAXPROG)&&(temp>=MINPROG)))
				{
				temp=MINPROG;
				}
			eeprom_write(EE_PROGRAM,temp);
			eeprom_write(EE_PROGRAM1,temp);
			eeprom_write(EE_PROGRAM2,temp);
			prog=temp;
			led_cnt=8*prog;
			step=sOFF;
			}
...
if((UIB[0]==CMND)&&(UIB[1]==9)&&(UIB[2]==10))
	{
	if((sub_ind==0)&&((prog==p3)||(prog==p4)))sub_ind=1;
	else sub_ind=0;
 	}

else if((UIB[0]==CMND)&&(UIB[1]==5)&&(UIB[2]==6))
	{
	temp=eeprom_read(adress);
	temp++;
	gran(&temp,0,200);
	eeprom_write(adress,(char)temp);
	}
...
/*prog=eeprom_read(EE_PROGRAM);

if(prog>MAXPROG)
	{
	prog=MAXPROG;
	eeprom_write(EE_PROGRAM,prog);
	}
if(prog<MINPROG)
	{
	prog=MINPROG;
...
while (1)
	...
	if(bit_10Hz)
        ...
        //	else if(prog==p2) OUT(3,CMND,eeprom_read(DELAY3),eeprom_read(DELAY4),0,0,0);
          ...
          //OUT(3,CMND,/*prog*/main_cnt,prog,0,0,0);
		}
	if(bit_1Hz)
		{
		bit_1Hz=0;
