//Программа "Пакетик полуавтомат длинный или короткий определяется дефайном _long_ или _short_"
//#define _long_
#define _short_
...
else if((UIB[0]==CMND)&&(UIB[1]==13)&&(UIB[2]==14))
	{
	temp=eeprom_read(DELAY3);
	temp++;
	eeprom_write(DELAY3,temp);
 	}
else if((UIB[0]==CMND)&&(UIB[1]==15)&&(UIB[2]==16))
	{
	temp=eeprom_read(DELAY3);
	temp--;
	eeprom_write(DELAY3,temp);
		...
		bit_10Hz=0;
         // out_out();
          ...
          prog_drv();
        	...
        	OUT(3,CMND,eeprom_read(DELAY1),eeprom_read(DELAY2),eeprom_read(DELAY3),0,0);
