...
if(main_loop_cmd==cmdSTART)
	...
	if(ee_prog==EE_PROG_ONLY_ORIENT)
		{
		orient_cmd=cmdSTART;
		}  
	...
	else if((ee_prog==EE_PROG_ONLY_MAIN_LOOP)||(ee_prog==EE_PROG_FULL))
		{
		main_loop_step=s1;
...
if(main_loop_step==sOFF)
	...
	if(main_loop_cnt_del==0)
		{
		if(ee_prog==EE_PROG_ONLY_MAIN_LOOP)
			{
			if(ee_loop_mode==elmAUTO)main_loop_cmd=cmdSTART;
...
//-----------------------------------------------
void orient_hndl(void)
{
if(orient_cmd==cmdSTART)
	{
	orient_step=s1;
	orient_cnt_del=0;
	bORIENT_COMPLETE=0;
...
if(orient_step!=sOFF)ind_out[4]&=~(1<<LED_ORIENT);
if(napoln_step!=sOFF)ind_out[4]&=~(1<<LED_NAPOLN);
if(payka_step!=sOFF)ind_out[4]&=~(1<<LED_PAYKA);
if(main_loop_step!=sOFF)ind_out[4]&=~(1<<LED_MAIN_LOOP);	
}
...
if(ind==iMn)
	...
	if(but==butP_)
		...
		{
		if(but==butR)ee_prog++;
		else if(but==butL)ee_prog--;
		if(ee_prog>5)ee_prog=0;
		if(ee_prog<0)ee_prog=5;
		}
	else if (sub_ind==1)
		{             
